  ...
  if (jsencoding.encodingExists(encoding)) {
    if (state.buffer !== undefined) {
      if (state.encoding === encoding && state.consecutive) {
        ...
        var newbuf = new Buffer.from(state.buffer.length + buf.length);
    ...
    if (!isPartial && nextBuf) {
      ...
      var lookahead, lookaheadBuf = new Buffer.from(buf.length + nextBuf.length);
...
function decodeWords(str, state) {
  ...
  for (i = 0, leni = replaces.length; i < leni; ++i) {
    ...
    if (m.encoding === 'q') {
      ...
      bytes = new Buffer.from(m.chunk.replace(RE_QENC, qEncReplacer), 'binary');
      next = undefined;
    } else {
      // base64
      bytes = m.buf || new Buffer.from(m.chunk, 'base64');
      next = replaces[i + 1];
      if (next && next.consecutive && next.encoding === m.encoding
        ...
        next.buf = new Buffer.from(next.chunk, 'base64');
      }
    }
